...
- **US-21:** As a user, I want to see "Request to Join" button for private events before sending a request
...
39. **FR-39:** For private events, the system must show a "Request to Join" button instead of a "Join Event" button to non-attendees (both on event list and event detail page)
...
50. **FR-50:** After sending a join request, the system must show "Request Pending" status on both the event list cards and the event detail page
51. **FR-51:** The system must prevent event organizers from seeing or using join/request buttons for their own events
52. **FR-52:** The system must prevent event organizers from requesting to join their own events (API-level validation)
53. **FR-53:** The system must fetch and display pending request status when loading events (both list and detail views)
...
12. **EC-12:** Event creator tries to request to join their own private event
    - **Solution:** Creator is automatically an attendee, hide join/request button
